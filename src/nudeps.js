...
export default class Nudeps {
	...
	/**
	 * Seed `toCopy` with top-level production dependencies that have a declared alias
	 * but produced no import-map entries (e.g. CSS/font-only packages like bootstrap-icons,
	 * which expose only `style`/`sass` and no `main`/`module`/`exports`).
	 *
	 * Alias materialization is otherwise driven entirely by the import-map walk in index.js:
	 * a package only lands in `toCopy` — and thus only gets copied + aliased — if it emits a
	 * `node_modules/…` URL into the map. A package with no JS entry point traces to zero entries,
	 * so its alias symlink would never be created and asset URLs (CSS, fonts) would 404. See #102.
	 *
	 * Restricted to declared production deps (skipping devDependencies and hoisted transitive
	 * packages, which are also top-level in node_modules) and honors `exclude`, so the default
	 * `alias: true` doesn't drag unrelated packages into the output.
	 *
	 * @param {Record<string, string>} toCopy - Map of source path → output dir, mutated in place.
	 * @returns {import("./util/package.js").default[]} The packages newly seeded this call.
	 */
	seedAliasedPackages (toCopy) {
		...
		for (let pkg of this.packages) {
			...
			if (!prodDeps.has(pkg.installName)) {
				continue; // declared production deps only (skip dev/transitive)
			}
			if (exclude.has(pkg.installName)) {
				continue; // honor exclude
			}
			if (this.aliases(pkg).length === 0) {
				continue; // must have a declared alias
			}
			if (pkg.path in toCopy) {
				continue; // already materialized via the import-map walk
			}
	...
	/**
	 * Object-form alias keys that don't match any installed package — likely a typo or a
	 * removed dependency. Such aliases are silent no-ops (they materialize nothing), so we
	 * surface them rather than letting them fail at runtime with a 404. See #102.
	 * @returns {string[]}
	 */
	unmatchedAliases () {
		let { alias } = this.config;

		if (!alias || typeof alias !== "object" || Array.isArray(alias)) {
			return [];
		}

		return Object.keys(alias).filter(key => !this.packages.has(key));
	}
